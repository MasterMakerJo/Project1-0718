...
//最后修订者:杨丽颖  时间：2017-07-17
//最后决定将判断方位的内容放入AddInPlaArr中去，这样简化了在help分支中的情况讨论
int RecommendCards::Help1Rocket(struct fDdz * pDdz)
{
	int *iCards = pSplitCards->startSplitCard(pDdz->iOnHand, 0);
	if (iCards[0]>-1){
		/*if(GetPlayerType(pDdz)==1||GetPlayerType(pDdz)==2)
		{
		     AddInPlaArr(pDdz, iCards);
		}
		if (GetPlayerType(pDdz) == 3 && iHandNum <= 3)
		{
			AddInPlaArr(pDdz, iCards);
		}*/
		AddInPlaArr(pDdz, iCards);
...
//--------------找不在双顺的对子，拆出来应对单牌-----------------------------
		...
		int shunPoint[3] = { -1, -1, -1 };//里面存的内容是一个三位数，百位是拥有的牌种类，个位是该双顺中牌面值最大的那张
		int ShunNum = 0;
		j = 0;
		...
		for (i = 0; iCards[i] != -1; i++)
		{
			...
			if (iCards[i] == -2)
			{
				int point = iCards[i - 1] / 4;//找到对子的点数
				for (j = 0; j < ShunNum; j++)//不超过3
				{
					int maxPoint = shunPoint[j] % 100;//最大点数
					int minPoint = maxPoint - shunPoint[j] / 100 + 1;//最小点数
...
int RecommendCards::Help11FourDouble(struct fDdz * pDdz)
{
	...
	int sidaiNum = 0;
	int i, iTempI, j, k;
	if (CInfo->CountCards(pDdz->iOnHand)<8)//手牌数小于8的时候
		return 0;
	CInfo->SortById(pDdz->iOnHand);//排序
	for (i = 0; pDdz->iOnHand[i + 3] >= 0; i++)
	{
		if (pDdz->iOnHand[i] / 4 > pDdz->iLastMainPoint
			&& pDdz->iOnHand[i] / 4 == pDdz->iOnHand[i + 1] / 4
...
int RecommendCards::AnalyzeSingleLink(int Length, int iLastMainPoint, int *Cards, int* iTmpShunzi)
{
	...
	for (i = 0; i < 21; i++)
	{
		iCards[i] = Cards[i];//就是iOnHand数组中的内容
	}
	CInfo->GetUnReaptedCards(iCards);//获得不重牌
...
int RecommendCards::deleteTypeLeftSingleCount(int*iCards, int*type)
{
